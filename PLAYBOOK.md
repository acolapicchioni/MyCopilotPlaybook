# PLAYBOOK — Human Guide for AI-Assisted Development

> **Scope note**: While the principles are universal, the concrete tooling and examples target Python projects.
>
> LLM-facing operational rules live in [`copilot-instructions.md`](./copilot-instructions.md).

## How This Playbook Works

This file is the human-facing reference for architecture rationale, workflow guidance, lessons learned, tooling baselines, and operational standards.

| File/Path | Audience | Purpose |
|---|---|---|
| `copilot-instructions.md` | LLMs | Terse execution directives only |
| `PLAYBOOK.md` | Humans | Rationale, examples, war stories, workflow, onboarding |
| `.github/instructions/*.instructions.md` | Copilot reviews/chats | Scoped instruction layers (`applyTo`) |
| `docs/adr/*.md` | Humans + LLMs | Architecture decision records |

### Maintenance Rules
- Keep `copilot-instructions.md` concise and directive-focused (<200 lines).
- Keep rationale, examples, and long-form guidance in `PLAYBOOK.md`.
- Record architecture changes as ADRs under `docs/adr/`.
- When LLM rules change, run `scripts/sync-ai-instructions.sh`.

---

## 🤖 Cross-Tool Compatibility

| AI Tool | Root File | Scoped/Hierarchical Support | Scoping Mechanism |
|---|---|---|---|
| **GitHub Copilot** | `.github/copilot-instructions.md` | ✅ Yes | `.github/instructions/*.instructions.md` with `applyTo` frontmatter |
| **Claude Code** | `CLAUDE.md` | ✅ Yes | `CLAUDE.md` files discovered in subdirectories |
| **OpenAI Codex** | `AGENTS.md` | ✅ Yes | `AGENTS.md` files discovered in subdirectories |
| **Gemini CLI** | `GEMINI.md` | ✅ Yes | `GEMINI.md` files discovered in subdirectories |
| **Cursor** | `.cursor/rules/*.mdc` | ✅ Yes | Rule files with glob frontmatter |
| **Windsurf** | `.windsurfrules` | ❌ No | Single file only |
| **Aider** | `.aider.conf.yml` | ❌ Manual | Config references explicit instruction files |

Use `scripts/sync-ai-instructions.sh` to propagate `copilot-instructions.md` to tool-specific root files.
